...
def generate_launch_description():
    ...
    if LaunchConfigurationEquals(robot_type, "c1"):
        params_file = os.path.join(
            get_package_share_directory("lidar_transform"),
            "config",
            "lidar_transform_params_c1.yaml",
        )
    else:
        params_file = os.path.join(
            get_package_share_directory("lidar_transform"),
    ...
    lidar_trans_publisher = Node(
        ...
        arguments=[*lidarOffsetList_str, "lio_base_link", "lidar"],
    )
    ...
    realsense_trans_publisher = Node(
        ...
        arguments=[*cameraOffsetList_str, "lio_base_link", "camera_link"],
    )
